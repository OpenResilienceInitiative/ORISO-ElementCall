...
export const computeUrlParams = (search = "", hash = ""): UrlParams => {
   ...
   */
  // Allow the caller to override the intent via URL parameters in both widget
  // and standalone (SPA) modes. In upstream Element Call this was limited to
  // widget usage only, but for our ORISO integration we deep-link into calls
  // via plain URLs, so we need to honour `?intent=...` even when `isWidget`
  // is false.
  const intent =
  ...
  const configuration: Partial<UrlConfiguration> = {
    ...
    // Allow skipLobby to be controlled via URL parameters in both widget and SPA
    // modes. In SPA mode this lets integrators (like ORISO) deep-link directly
    // into an active call without showing the intermediate "Join call" lobby.
    skipLobby: parser.getFlag("skipLobby"),
    // In SPA mode the user historically always exited to the home screen when
    // hanging up rather than being sent back to the lobby. To preserve that
    // behavior we keep `returnToLobby` gated on widget mode unless explicitly
    // set via URL params in the future.
